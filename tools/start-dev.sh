...
if [[ "$PROFILE" == "https" ]]; then
  PORT_IAS=7010; PORT_API=8010; PORT_CLIENT=6040
else
  PORT_IAS=7011; PORT_API=8012; PORT_CLIENT=5040
fi
...
echo "==> Opening window: IdentityAccessServer ..."
open_terminal "IdentityAccessServer" \
  "dotnet run --project '$ROOT/IdentityAccessServer/IdentityAccessServer.csproj' --launch-profile $PROFILE"

wait_for_port $PORT_IAS "IdentityAccessServer"
...
echo ""
echo "All three services started in separate Terminal windows."