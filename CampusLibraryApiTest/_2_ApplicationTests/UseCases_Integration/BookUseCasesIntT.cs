...
public sealed class BookUseCasesIntT : TestBaseIntegration {
   ...
   [Fact]
   public async Task AssignAuthorAsync_ok_persists_book_author_relationship() {
      using var scope = Root.CreateDefaultScope();
      var ct = TestContext.Current.CancellationToken;
      var useCases = scope.ServiceProvider.GetRequiredService<IBookUseCases>();
      ...
      resultAssignAuthor.IsSuccess.Should().BeTrue();
      ...
      actualBookDetailDto.Authors
         .Should()
         .ContainSingle(a => a.Id == author1.Id);
   ...
   public async Task AssignAuthorAsync_duplicate_author_fails() {
      ...
      // Assert
      resultAssignAuthor.IsFailure.Should().BeTrue();
      resultAssignAuthor.Error.Should().Be(CatalogErrors.AuthorAlreadyAssigned);
         
   }

   [Fact]
   public async Task AssignAuthorAsync_unknown_author_fails() {
      using var scope = Root.CreateDefaultScope();
      var ct = TestContext.Current.CancellationToken;
      var useCases = scope.ServiceProvider.GetRequiredService<IBookUseCases>();
      var bookRepository = scope.ServiceProvider.GetRequiredService<IBookRepository>();
      ...
      // Assert
      resultAssignAuthor.IsFailure.Should().BeTrue();
      resultAssignAuthor.Error.Should().Be(CatalogErrors.AuthorNotFound);
         
   }

   [Fact]
   public async Task AssignAuthorAsync_unknown_book_fails() {
      using var scope = Root.CreateDefaultScope();
      var ct = TestContext.Current.CancellationToken;
      var useCases = scope.ServiceProvider.GetRequiredService<IBookUseCases>();
      ...
      // Assert
      resultAssignAuthor.IsFailure.Should().BeTrue();
      resultAssignAuthor.Error.Should().Be(CatalogErrors.BookNotFound);
